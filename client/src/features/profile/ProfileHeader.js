    ...
    return (
        <section className="bg-indigo-600 flex flex-col gap-4 md:gap-5 sm:flex-row justify-between items-center p-6 rounded shadow-lg">
                ...
                <div className="flex gap-5 text-slate-300 text-xl text-center ">
                    <Link className="bg-indigo-900 rounded shadow text-sm p-1 md:text-md md:p-2" to='/profile/followers'>
                        <h3>Followers</h3>
                        <span>{profile.followers.length}</span>
                    </Link>
                    <Link className="bg-indigo-900 rounded shadow text-sm p-1 md:text-md md:p-2" to='/profile/following'>
                        <h3>Following</h3>
                        <span>{profile.following.length}</span>
                    </Link>
                </div>
                <Link className="flex items-center gap-3 bg-indigo-900 text-slate-300 text-md p-1 lg:text-lg lg:py-2 lg:px-2 rounded shadow" to='/more-info'>
                    <p>More Info</p>
